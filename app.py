...
def match_ingredients(ocr_text):
    ...
    # EXTRA LOGIC FOR FLUORIDE VALUE
    fluoride_match = re.search(
        r"f[^0-9]{0,5}(\d+[\.,]?\d*)",
        text_lower
    )
    ...
    if fluoride_match:
        ...
        value = float(
            fluoride_match.group(1).replace(",", ".")
        )
    ...
    if ph_match:
        ...
        value = float(
            ph_match.group(1).replace(",", ".")
        )

        if value >= 8.5:

            already_exists = any(
                x["name"] == "High pH / Very Alkaline"
                for x in found
            )
...
def overall_score(ingredients):
    ...
    danger = sum(
        ...
        if i["risk_level"] == "DANGER"
    )

    warning = sum(
        1 for i in ingredients
        if i["risk_level"] == "WARNING"
    )
